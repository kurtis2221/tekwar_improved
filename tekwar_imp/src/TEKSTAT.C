...
void     enemynewsector(int i, int oldsectnum);
...
hideplea(short sn, short ext)
{
     ...
     case SARAHWALKPIC:
     case SAMWALKPIC:
          playsound(S_DIANE_DONTSHOOTP+(krand_intercept("STAT1397")%7),sprptr[sn]->x,sprptr[sn]->y,0,ST_UNIQUE);
...
fleescream(short sn, short ext)
{
     ...
     switch( sprXTptr[ext]->basepic ) {
     case ERWALKPIC:
          playsound(S_MALE_OHMYGOD+(krand_intercept("STAT1424")%12),sprptr[sn]->x,sprptr[sn]->y,0,ST_UNIQUE);
     ...
     case SAMWALKPIC:
		...
		case 0:
          case 2:
               playsound(S_FEM_RUNHEGOTGUN+(krand_intercept("STAT1434")%12),sprptr[sn]->x,sprptr[sn]->y,0,ST_UNIQUE);
               break;
          default:
...
void
enemynewsector(int i, int oldsectnum)
{
	int	ext=sprptr[i]->extra;
...
ambushyell(short sn, short ext)
{
     ...
     case ANTWALKPIC:
     case RUBWALKPIC:
          playsound(S_MALE_COMEONYOU+(krand_intercept("STAT2603")%7),sprptr[sn]->x,sprptr[sn]->y,0,ST_IMMEDIATE);
          sprXTptr[ext]->aimask|=AI_DIDAMBUSHYELL;
          break;
     case DIWALKPIC:
          playsound(S_DIM_WANTSOMETHIS+(krand_intercept("STAT2607")%5),sprptr[sn]->x,sprptr[sn]->y,0,ST_IMMEDIATE);
          sprXTptr[ext]->aimask|=AI_DIDAMBUSHYELL;
          break;
     case MAWALKPIC: 
          playsound(S_MAR_THINKYOUTAKE+(krand_intercept("STAT2609")%3),sprptr[sn]->x,sprptr[sn]->y,0,ST_IMMEDIATE);
          sprXTptr[ext]->aimask|=AI_DIDAMBUSHYELL;
          break;
     }
}
...
givewarning(short i, short ext)
{
     ...
     switch( sprXTptr[ext]->basepic ) {
     case COP1WALKPIC:
          playsound(S_GRD_WHATDOINGHERE+(krand_intercept("STAT2636")%5),sprptr[i]->x,sprptr[i]->y,0,ST_UNIQUE);
          sprXTptr[ext]->aimask|=AI_GAVEWARNING;
          break;
     }
}
