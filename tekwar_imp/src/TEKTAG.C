...
teknewsector(short p)
{
     ...
     if( sectptr[cursectnum[p]]->lotag == 4 && sectptr[ocursectnum[p]]->lotag != 4) {
...
tekswitchtrigger(short snum)
{
	...
	if (j == SWITCH0ON) sprite[neartagsprite].picnum = SWITCH0OFF;
	if (j == SWITCH0OFF) sprite[neartagsprite].picnum = SWITCH0ON;
