	...
	for(i=0;i<numsectors;i++)
	{
		switch(sector[i].lotag)
		{
			case 4:
				...
				break;
               case 25:
                    if( (singlemapmode != 0) || (generalplay != 0) ) {
                         sector[i].lotag=0;
                    }
                    break;
			case 10:
                    if( (generalplay == 0) && (warpsectorcnt < 64) ) {
...
teknetpickmap(void)
{
     ...
     if (switchlevelsflag) {
          strcpy(boardfilename,"NETC1.MAP");
     }
     setgamemode();
     initpaletteshifts();
     ...
     if (keystatus[0x1C] || keystatus[0x9C]) {
          keystatus[0x1C]=0;
		  mission = map;
          sprintf(boardfilename,"NETC%d.MAP",map+1);
     }
